    ...
    try:
        chat_response = mistral_client.chat.complete(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"Напиши увлекательный пост, который вызывает эмоции на тему: '{prompt}', используй эмодзи и добавь хэштеги",
                },
            ],
            temperature=0.7,
        )
...
def send_welcome(message):
    ...
    bot.reply_to(
        message,
        "Привет, Neo! Я твой личный бот-генератор контента на основе Mistral AI. "
        "Я могу создавать текстовые посты и изображения по твоему запросу. "
        "Чтобы начать, нажми кнопку 'Генерировать пост 🚀' или просто отправь мне тему для поста.",
        reply_markup=markup,
    )
...
def process_prompt_step(message):
    ...
    try:
        ...
        if not generated_text:
            bot.send_message(
                chat_id,
                "Не удалось сгенерировать текст поста. Попробуй еще раз, возможно, проблема с API ключом.",
            )
            return
        ...
        bot.send_message(
            chat_id, "Произошла непредвиденная ошибка. Пожалуйста, попробуй еще раз."
        )
...
def run_bot():
    ...
    """
    logger.info("Бот запущен и готов принимать сообщения, Neo...")
    bot.remove_webhook()
